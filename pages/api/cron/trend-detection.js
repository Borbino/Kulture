...
import { collectAllTrends, checkTrendLifecycle, updateTrendDatabase } from '../../../lib/trendManagement';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(`[Trend Detection] Database updated: ${updateResult.added} added, ${updateResult.updated} updated`);
    ...
    for (const keyword of customKeywords) {
      if (!TRACKING_ISSUES.some((i) => i.keyword === keyword)) {
        try {
          ...
          console.error(`[Trend Detection] Failed to track custom keyword "${keyword}":`, error.message);
        }
      }
    }
