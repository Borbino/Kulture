// [수익화 스키마] Kulture.wiki — 애드센스 수익 극대화 및 SEO 자동화 기사 문서
...
const postSchema = {
  ...
  fields: [
    ...
    },
    {
      ...
      title: '숨김 처리',
      type: 'boolean',
      description: 'AI 1차 필터 또는 누적 신고에 의한 자동 숨김 — true 시 프론트엔드 미노출',
      initialValue: false,
    },
    ...
    // ─────────────────────────────────────────────────
    // 7. 수익화 제어 — 제휴 링크 CEO 승인 (V15.1 정책 2)
    // ─────────────────────────────────────────────────
    {
      name: 'affiliateLinks',
      title: '제휴 링크 목록',
      type: 'array',
      description: 'AI가 제안한 제휴 링크 — isAffiliateApproved가 true일 때만 프론트엔드에서 렌더링',
      of: [
        {
          type: 'object',
          name: 'affiliateLink',
          ...
          fields: [
            {
              ...
              title: '링크 레이블',
              type: 'string',
              validation: Rule => Rule.required().error('레이블을 입력하세요.'),
            },
            {
              name: 'url',
              title: 'URL',
              ...
              validation: Rule =>
                Rule.required()
                  .uri({ scheme: ['https'] })
                  .error('유효한 HTTPS URL을 입력하세요.'),
            },
            {
              name: 'platform',
              title: '플랫폼',
              type: 'string',
              description: '예: Amazon, Coupang, Rakuten',
            },
          ],
          preview: {
            select: { label: 'label', url: 'url', platform: 'platform' },
            prepare({ label, url, platform }) {
              return { title: `🔗 ${label}`, subtitle: `[${platform ?? '?'}] ${url}` }
            },
          },
        },
      ],
    },
    {
      ...
      title: '제휴 링크 CEO 승인 여부',
      type: 'boolean',
      description: 'false(기본값) = pending — CEO가 Sanity Studio에서 true로 전환 시 프론트엔드 노출',
      initialValue: false,
    },
