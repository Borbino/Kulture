...
async function fetchXiaohongshuData(keyword, _options = {}) {
  try {
    ...
    logger.info('[Xiaohongshu] 공식 API 제한으로 웹 스크래핑 또는 제3자 서비스 필요')

    // 대안: RapidAPI의 Xiaohongshu API 사용
    const RAPIDAPI_KEY = process.env.RAPIDAPI_KEY

    if (!RAPIDAPI_KEY) {
      logger.info('[Xiaohongshu] RapidAPI key not configured, skipping')
    ...
    logger.error('[Xiaohongshu] Fetch failed:', error.message)
