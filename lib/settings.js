...
export async function getSiteSettings() {
  try {
    ...
    if (!settings) {
      logger.warn('[Settings]', 'No settings found in Sanity, using defaults')
    ...
    logger.error('[Settings]', `Error fetching settings: ${error.message}`)
    return DEFAULT_SETTINGS
  }
}
...
export async function updateSiteSettings(updates) {
  try {
    ...
    logger.error('[Settings]', `Error updating settings: ${error.message}`)
    throw error
  }
}
