...
export async function extractUserInterests(userId) {
  try {
    ...
    logger.error('[AIRecommendation]', `Error extracting user interests: ${error.message}`);
...
export async function getPersonalizedRecommendations(userId, limit = 10) {
  try {
    ...
    logger.error('[AIRecommendation]', `Error getting personalized recommendations: ${error.message}`);
    return [];
  }
}
...
export async function getSimilarPosts(postId, limit = 5) {
  try {
    ...
    logger.error('[AIRecommendation]', `Error getting similar posts: ${error.message}`);
    return [];
  }
}
...
export async function getTrendingPosts(timeRange = '24h', limit = 10) {
  try {
    ...
    logger.error('[AIRecommendation]', `Error getting trending posts: ${error.message}`);
    return [];
  }
}

/**
 * AI 기반 태그 추천 (generateWithBestModel 우선, 키워드 기반 폴백)
 */
export async function suggestTags(title, content) {
  const snippet = `${title} ${content}`.substring(0, 800);
  try {
    const aiResult = await generateWithBestModel(
      `You are a K-Culture content tagging assistant. Based on the following post, suggest 3-8 relevant tags.\nRespond ONLY in JSON array format: ["tag1", "tag2", ...].\n\nPost:\n"${snippet.replace(/"/g, "'")}"`,
      { maxTokens: 100, preferFree: true }
    );
    const arrayMatch = aiResult.text.match(/\[[\s\S]*?\]/);
    if (arrayMatch) {
      const parsed = JSON.parse(arrayMatch[0]);
      if (Array.isArray(parsed) && parsed.length > 0) {
        return parsed.map(t => String(t).toLowerCase().trim()).filter(Boolean).slice(0, 8);
      }
    }
  } catch {
    // 폴백: 키워드 기반
  }
  return fallbackSuggestTags(title, content);
}

/**
 * 키워드 기반 태그 추출 (폴백 전용)
 */
function fallbackSuggestTags(title, content) {
...
/**
 * AI 기반 카테고리 추천 (generateWithBestModel 우선, 텍스트 매칭 폴백)
 */
export async function suggestCategories(title, content, tags = []) {
  try {
    ...
    if (categories.length > 0) {
      ...
      const snippet = `${title} ${content} ${tags.join(' ')}`.substring(0, 600);
      try {
        const aiResult = await generateWithBestModel(
          `You are a K-Culture content category classifier. Choose up to 3 most relevant categories for the following post from this list: [${catNames}].\nRespond ONLY in JSON array format: ["Category1", "Category2"].\n\nPost:\n"${snippet.replace(/"/g, "'")}"`,
          { maxTokens: 100, preferFree: true }
        );
        const arrayMatch = aiResult.text.match(/\[[\s\S]*?\]/);
        if (arrayMatch) {
          const chosen = JSON.parse(arrayMatch[0]);
          if (Array.isArray(chosen) && chosen.length > 0) {
            return categories
              .filter(cat => chosen.some(name => cat.title.toLowerCase() === String(name).toLowerCase()))
              .slice(0, 3)
      ...
      const scored = categories.map((cat) => {
        ...
        tags.forEach((tag) => {
          if (cat.title.toLowerCase().includes(tag.toLowerCase())) score += 5;
        });
        return { ...cat, score };
      });
      return scored
        ...
        .slice(0, 3)
        .map(({ _id, title: catTitle, slug }) => ({ _id, title: catTitle, slug }));
    }
    return [];
  } catch (error) {
    logger.error('[AIRecommendation]', `Error suggesting categories: ${error.message}`);
    return [];
  }
}
