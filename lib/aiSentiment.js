/**
 * AI 감정 분석 시스템
 * - 댓글 감정 분석 (긍정/부정/중립)
 * - 독성 콘텐츠 감지
 * - 스팸 감지
 * - 다국어 지원: 한국어, 영어, 일본어(JP), 중국어(ZH)
 * [업그레이드] generateWithBestModel() 기반 AI 분석 (사전기반 폴백 보존)
 */

import { generateWithBestModel } from './aiModelManager.js';

// ========== 다국어 감성 사전 (폴백용) ==========

const SENTIMENT_DICT = {
  positive: {
    ko: ['좋아', '최고', '훌륭', '멋지', '완벽', '감동', '행복', '사랑', '예쁘', '아름답', '고마', '감사', '축하', '대단', '놀라', '신나', '재밌', '웃겨', '즐거', '최애', '설레', '뿌듯'],
    en: ['good', 'great', 'excellent', 'amazing', 'perfect', 'wonderful', 'love', 'beautiful', 'thank', 'happy', 'joy', 'awesome', 'fantastic', 'brilliant', 'incredible', 'stunning'],
    ja: ['好き', '最高', '素晴らしい', 'かっこいい', '可愛い', '完璧', '感動', '幸せ', '嬉しい', '楽しい', 'ありがとう', 'すごい', '最強', 'きれい', '大好き', '癒し'],
    zh: ['好', '棒', '太棒了', '漂亮', '完美', '感动', '幸福', '爱', '喜欢', '谢谢', '厉害', '很美', '可爱', '精彩', '好听', '大神'],
  },
  negative: {
    ko: ['싫어', '최악', '별로', '실망', '화나', '짜증', '속상', '슬프', '우울', '미워', '나쁘', '더러', '역겨', '끔찍', '무서', '답답', '불편', '지겹'],
    en: ['bad', 'terrible', 'awful', 'hate', 'angry', 'sad', 'disappointed', 'horrible', 'disgusting', 'annoying', 'frustrating', 'boring', 'poor', 'worst', 'pathetic'],
    ja: ['嫌い', '最悪', 'つまらない', '失望', '怖い', '汚い', '悲しい', 'うざい', '怒り', 'ひどい', '気持ち悪い', '嫌', 'ムカつく'],
    zh: ['差', '糟糕', '讨厌', '烦', '失望', '可怕', '难看', '垃圾', '烂', '恶心', '无聊', '生气', '伤心'],
  },
  toxic: {
    ko: ['죽어', '꺼져', '병신', '미친', '씨발', '개새', '좆', '니미', '지랄'],
    en: ['fuck', 'shit', 'bitch', 'asshole', 'damn', 'idiot', 'stupid', 'dumb', 'kill yourself'],
    ja: ['死ね', 'キモい', 'うせろ', 'ばか', 'アホ', 'クソ'],
    zh: ['去死', '傻逼', '垃圾死', '滚'],
  },
};

/**
 * 텍스트 언어 감지 (빠른 휴리스틱 방식)
 */
export function detectLanguage(text) {
  const sample = text.slice(0, 200);
  if (/[\u4e00-\u9fff]/.test(sample) && !/[\u3040-\u309f\u30a0-\u30ff]/.test(sample)) return 'zh';
  if (/[\u3040-\u309f\u30a0-\u30ff\u4e00-\u9fff]/.test(sample)) return 'ja';
  if (/[\uac00-\ud7af]/.test(sample)) return 'ko';
  return 'en';
}

/**
 * 사전 기반 감정 분석 (폴백 전용)
 */
function heuristicSentiment(text, langHint) {
  const content = text.toLowerCase();
  const lang = langHint || detectLanguage(text);

  // 현재 언어 + 영어 공통으로 검사 (영어는 글로벌 공통)
  const langsToCheck = lang === 'en' ? ['en'] : [lang, 'en'];

  // 긍정 키워드
  const positiveWords = langsToCheck.flatMap(l => SENTIMENT_DICT.positive[l] || []);
  // 부정 키워드
  const negativeWords = langsToCheck.flatMap(l => SENTIMENT_DICT.negative[l] || []);
  // 독성 키워드
  const toxicWords = langsToCheck.flatMap(l => SENTIMENT_DICT.toxic[l] || []);

  let positiveScore = 0;
  let negativeScore = 0;
  let toxicScore = 0;

  // 키워드 매칭
  positiveWords.forEach(word => {
    const regex = new RegExp(word, 'gi');
    const matches = content.match(regex);
    if (matches) positiveScore += matches.length;
  });

  negativeWords.forEach(word => {
    const regex = new RegExp(word, 'gi');
    const matches = content.match(regex);
    if (matches) negativeScore += matches.length;
  });

  toxicWords.forEach(word => {
    const regex = new RegExp(word, 'gi');
    const matches = content.match(regex);
    if (matches) toxicScore += matches.length * 3; // 독성 키워드는 가중치 높게
  });

  // 감정 판단
  let sentiment = 'neutral';
  if (positiveScore > negativeScore + 2) {
    sentiment = 'positive';
  } else if (negativeScore > positiveScore + 2) {
    sentiment = 'negative';
  }

  // 독성 판단
  const isToxic = toxicScore > 2;

  // 점수 정규화 (0-1 범위)
  const totalScore = positiveScore + negativeScore + toxicScore || 1;
  const normalized = {
    positive: positiveScore / totalScore,
    negative: negativeScore / totalScore,
    toxic: toxicScore / totalScore,
  };

  return {
    sentiment, // 'positive', 'negative', 'neutral'
    isToxic,
    detectedLanguage: lang,
    scores: {
      positive: positiveScore,
      negative: negativeScore,
      toxic: toxicScore,
    },
    normalized,
    confidence: Math.abs(positiveScore - negativeScore) / totalScore,
    provider: 'heuristic',
  };
}

/**
 * 다국어 감정 분석 (AI 우선, 사전기반 폴백)
 * - generateWithBestModel() 로 LLM 기반 감성 분석 시도
 * - API 키 미설정 또는 오류 시 heuristicSentiment() 폴백
 */
export async function analyzeSentiment(text, langHint = null) {
  const snippet = text.substring(0, 600);
  try {
    const aiResult = await generateWithBestModel(
      `Analyze the sentiment of the following text. Respond ONLY in valid JSON with these exact keys: {"sentiment": "positive|negative|neutral", "isToxic": true|false, "confidence": <0.0-1.0>, "detectedLanguage": "ko|en|ja|zh|other"}.\n\nText: "${snippet.replace(/"/g, "'")}"`
      , { maxTokens: 120, preferFree: true }
    );
    const jsonMatch = aiResult.text.match(/\{[\s\S]*\}/);
    if (jsonMatch) {
      const parsed = JSON.parse(jsonMatch[0]);
      if (['positive', 'negative', 'neutral'].includes(parsed.sentiment)) {
        return {
          sentiment: parsed.sentiment,
          isToxic: Boolean(parsed.isToxic),
          detectedLanguage: parsed.detectedLanguage || detectLanguage(text),
          scores: { positive: 0, negative: 0, toxic: 0 },
          normalized: { positive: 0, negative: 0, toxic: 0 },
          confidence: typeof parsed.confidence === 'number' ? parsed.confidence : 0.8,
          provider: aiResult.providerName || 'ai',
        };
      }
    }
  } catch {
    // 폴백: 사전기반
  }
  return heuristicSentiment(text, langHint);
}

/**
 * 스팸 감지
 */
export function detectSpam(text, userPostCount = 0, userLevel = 1) {
  const content = text.toLowerCase();
  let spamScore = 0;

  // 스팸 패턴 체크
  const spamPatterns = [
    /https?:\/\/[^\s]+/gi, // URL 포함
    /\d{3}-\d{4}-\d{4}/g, // 전화번호
    /[가-힣]{2,}\s*\d+원/, // 가격 정보
    /클릭|광고|홍보|이벤트|할인|무료|당첨/gi, // 스팸 키워드
    /(.)\1{4,}/g, // 같은 문자 5번 이상 반복
  ];

  spamPatterns.forEach(pattern => {
    const matches = content.match(pattern);
    if (matches) {
      spamScore += matches.length * 2;
    }
  });

  // 특수문자 과다 사용
  const specialChars = content.match(/[!@#$%^&*()_+=[\]{};':"\\|,.<>?]/g);
  if (specialChars && specialChars.length > 10) {
    spamScore += 3;
  }

  // 전체 대문자 (영어)
  const upperCaseRatio = (content.match(/[A-Z]/g) || []).length / content.length;
  if (upperCaseRatio > 0.5 && content.length > 20) {
    spamScore += 2;
  }

  // 신규 사용자는 스팸 가능성 높음
  if (userPostCount < 5 || userLevel < 2) {
    spamScore += 1;
  }

  const isSpam = spamScore >= 5;

  return {
    isSpam,
    score: spamScore,
    confidence: Math.min(spamScore / 10, 1),
  };
}

/**
 * 댓글 품질 분석
 */
export async function analyzeCommentQuality(comment, postContent = '') {
  const sentiment = await analyzeSentiment(comment);
  const spam = detectSpam(comment);

  // 댓글 길이
  const length = comment.length;
  const wordCount = comment.split(/\s+/).length;

  // 품질 점수 계산
  let qualityScore = 50; // 기본 50점

  // 긍정적인 감정 (+10)
  if (sentiment.sentiment === 'positive') qualityScore += 10;

  // 부정적이거나 독성 (-20)
  if (sentiment.sentiment === 'negative' || sentiment.isToxic) qualityScore -= 20;

  // 스팸 (-30)
  if (spam.isSpam) qualityScore -= 30;

  // 적절한 길이 (20-500자) (+10)
  if (length >= 20 && length <= 500) qualityScore += 10;

  // 너무 짧음 (-10)
  if (length < 10) qualityScore -= 10;

  // 단어 수 (5개 이상) (+5)
  if (wordCount >= 5) qualityScore += 5;

  // 게시글 내용과 관련성 (간단한 키워드 매칭)
  if (postContent) {
    const postWords = postContent.toLowerCase().split(/\s+/).slice(0, 50);
    const commentWords = comment.toLowerCase().split(/\s+/);
    const overlap = commentWords.filter(word => postWords.includes(word)).length;
    if (overlap > 3) qualityScore += 10;
  }

  // 0-100 범위로 제한
  qualityScore = Math.max(0, Math.min(100, qualityScore));

  return {
    score: qualityScore,
    sentiment: sentiment.sentiment,
    isToxic: sentiment.isToxic,
    isSpam: spam.isSpam,
    length,
    wordCount,
    recommendations: {
      needsModeration: sentiment.isToxic || spam.isSpam || qualityScore < 30,
      autoApprove: qualityScore >= 70 && !sentiment.isToxic && !spam.isSpam,
    },
  };
}

/**
 * 게시글 품질 분석
 */
export async function analyzePostQuality(title, content, tags = []) {
  let qualityScore = 50;

  // 제목 길이 (10-100자)
  if (title.length >= 10 && title.length <= 100) {
    qualityScore += 10;
  } else if (title.length < 10) {
    qualityScore -= 15;
  }

  // 내용 길이 (100자 이상)
  if (content.length >= 100) {
    qualityScore += 15;
  } else if (content.length < 50) {
    qualityScore -= 10;
  }

  // 태그 개수 (1-5개)
  if (tags.length >= 1 && tags.length <= 5) {
    qualityScore += 10;
  }

  // 감정 및 스팸 분석
  const contentSentiment = await analyzeSentiment(content);
  const contentSpam = detectSpam(content);

  if (contentSentiment.isToxic) qualityScore -= 30;
  if (contentSpam.isSpam) qualityScore -= 40;

  // 문장 구조 (문장 개수)
  const sentences = content.split(/[.!?]/).filter(s => s.trim().length > 0);
  if (sentences.length >= 3) qualityScore += 10;

  // 0-100 범위로 제한
  qualityScore = Math.max(0, Math.min(100, qualityScore));

  return {
    score: qualityScore,
    sentiment: contentSentiment.sentiment,
    isToxic: contentSentiment.isToxic,
    isSpam: contentSpam.isSpam,
    metrics: {
      titleLength: title.length,
      contentLength: content.length,
      tagCount: tags.length,
      sentenceCount: sentences.length,
    },
    recommendations: {
      needsModeration: contentSentiment.isToxic || contentSpam.isSpam || qualityScore < 40,
      autoApprove: qualityScore >= 70 && !contentSentiment.isToxic && !contentSpam.isSpam,
      suggestions: [],
    },
  };
}
