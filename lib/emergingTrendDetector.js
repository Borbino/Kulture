...
import { alertEmergingEntity } from './alerting.js';
  ...
  for (const entity of emergingEntities.slice(0, 30)) {
    try {
      ...
      if (entity.shouldAlert) {
        ...
        // Slack / Discord 실시간 알림 발송
        alertEmergingEntity(entity).catch(e =>
          logger.error('[emerging]', 'alertEmergingEntity failed', { error: e.message })
        );
        alerted++;
      }
    } catch (error) {
