...
async function fetchGoogleTrends() {
  try {
    ...
    for (let i = 1; i < titles.length; i++) {
      const traffic = traffics[i - 1] ? parseInt(traffics[i - 1].replace(/[^0-9]/g, ''), 10) : 1000
      trends.push({
        keyword: titles[i],
    ...
    logger.error('[trend]', '[Google Trends] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchBingTrends() {
  try {
    ...
    logger.error('[trend]', '[Naver News] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchTwitterTrends() {
  ...
  try {
    ...
    logger.error('[trend]', '[Twitter Trends] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchYouTubeTrends() {
  ...
  try {
    ...
    logger.error('[trend]', '[YouTube Trends] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchRedditTrends() {
  ...
  try {
    ...
    logger.error('[trend]', '[Reddit Trends] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchInstagramTrends() {
  try {
    ...
    logger.error('[trend]', '[Instagram Trends] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchTikTokTrends() {
  try {
    ...
    logger.error('[trend]', '[TikTok Trends] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchWeiboTrends() {
  try {
    ...
    logger.error('[trend]', '[Weibo Trends] Fetch failed:', error.message)
    return []
  }
}
...
async function fetchBilibiliTrends() {
  try {
    ...
    logger.error('[trend]', '[Bilibili Trends] Fetch failed:', error.message)
    return []
  }
}
...
export async function collectAllTrends() {
  ...
  logger.info('[trend]', 'Starting multi-source trend collection...')
  ...
  logger.info('[trend]', `[Trend Collection] Collected ${allTrends.length} raw trends from ${9} sources`)
  ...
  logger.info('[trend]', `Completed in ${elapsed}ms. ${sorted.length} validated trends.`)
...
export async function checkTrendLifecycle() {
  ...
  for (const trend of existingTrends) {
    ...
    logger.info('[trend]', `[Lifecycle] Removed ${toDelete.length} obsolete trends`)
    ...
    logger.error('[trend]', '[Lifecycle] Check failed:', error.message)
...
export async function updateTrendDatabase(trends) {
  ...
  for (const trend of trends) {
    try {
      ...
      logger.error('[trend]', `[Update] Failed for trend "${trend.keyword}":`, error.message)
      results.skipped++
    }
  }
