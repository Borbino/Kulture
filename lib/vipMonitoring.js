...
export async function monitorVIP(vipId) {
  ...
  basicResults
    .filter(r => r !== null)
    .forEach(result => {
      const platformKey = (result && typeof result === 'object' && 'platform' in result) 
        ? result.platform || 'unknown'
        : 'unknown'
      allPlatforms[platformKey] = result
    })
...
async function searchTwitter(keywords) {
  ...
  try {
    const query = keywords.slice(0, 3).join(' OR ') // 상위 3개 키워드만
