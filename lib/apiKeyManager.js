...
export class APIKeyManager {
  ...
  trackUsage(service, amount = 1) {
    ...
    let limit = API_LIMITS[service]
    if (!limit) {
      logger.warn('APIKeyManager', `Unknown service: ${service} - tracking without limit`)
      // 알 수 없는 서비스도 추적 (분석용)
      limit = Infinity
    }
    ...
    if (current + amount > limit * 0.9) {
      const percentage = Math.round(((current + amount) / limit) * 100)
      logger.warn(
        'APIKeyManager',
        `${service} is at ${percentage}% (${current + amount}/${limit})`
      )
    }

    // 100% 도달 시 알림
    if (current + amount >= limit) {
      logger.error('APIKeyManager', `${service} has reached its daily limit (${limit})`)
    }
  }
  ...
  resetDailyQuotas() {
    ...
    logger.info('APIKeyManager', 'Daily quotas reset')
  }
