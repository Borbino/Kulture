...
export class LogAggregator {
  constructor() {
    ...
    this.maxLogs = 1000 // 최대 로그 보관 수 (메모리 누수 방지)
    this.lastAlertCount = 0 // 마지막 알림 시점 에러 수
  }
  ...
  logError(module, error, context = {}) {
    ...
    // 임계값 도달 시 알림 (중복 알림 방지)
    if (this.errors.length >= this.alertThreshold && this.lastAlertCount < this.alertThreshold) {
      this.sendAlert('Multiple errors detected', this.errors.length)
      this.lastAlertCount = this.errors.length
    }
  ...
  sendAlert(message, errorCount) {
    logger.error(`[Log Aggregator Alert] ${message} (${errorCount} errors)`)
