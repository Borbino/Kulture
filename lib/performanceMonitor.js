...
class PerformanceMonitor {
  ...
  printReport() {
    ...
    logger.info(`Period: ${report.period.start} ~ ${report.period.end}`)
    logger.info(`Duration: ${report.period.durationMinutes} minutes`)
    logger.info('\n[Summary]')
    logger.info(`Total API Calls: ${report.summary.totalApiCalls}`)
    logger.info(`Total Errors: ${report.summary.totalErrors}`)
    logger.info(`Avg Cache Hit Rate: ${report.summary.avgCacheHitRate}%`)
    ...
    if (report.apis.length > 0) {
      ...
      report.apis.forEach(api => {
        ...
        logger.info(`    Error Rate: ${api.errorRate}%`)
      })
    }
    ...
    if (report.errors.length > 0) {
      ...
      report.errors.forEach(error => {
        logger.info(`  ${error.source}: ${error.count} errors`)
        error.recentErrors.forEach(e => {
          logger.info(`    - [${e.timestamp}] ${e.message}`)
        })
      })
    }
