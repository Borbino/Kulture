...
/**
 * AI 모델매니저를 활용한 컨텍스트 인식 고품질 번역
 * OpenAI/Gemini/Claude 등 가장 성능 좋은 무료 모델을 자동 선택
 */
async function translateWithAI(text, sourceLang, targetLang, context = '') {
  ...
  let systemInstruction = `You are a world-class professional translator with expertise in ${sourceLanguageName} and ${targetLanguageName}.
...
5. Ensure natural, native-level fluency in the target language`;
  ...
  const result = await generateWithBestModel(prompt, {
    maxTokens: Math.min(Math.ceil(text.length * 2.5), 4000),
    preferFree: true,
  });
...
/**
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * 다이내믹 AI 라우터 — 4단 폴백 번역 엔진 (Phase 8)
 *
 *   1순위: DeepL Free Tier  (전문 번역 엔진, 최고 품질)
 *   2순위: Claude 3.5 Sonnet (HTTP 456 할당량 초과 시 즉시 우회)
 *   3순위: GPT-4o            (Claude 실패 시)
 *   4순위: Gemini 1.5 Flash  (GPT-4o 실패 시)
 *   5순위: AI Auto-Select    (aiModelManager 동적 발견)
 *   6순위: Google Translate  (최후 수단)
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 */
 ...
 * 번역용 LLM 프롬프트 공통 빌더
 */
function buildTranslationPrompt(text, sourceLangName, targetLangName, context = '') {
  let p = `You are a world-class professional translator from ${sourceLangName} to ${targetLangName}.\nCRITICAL RULES:\n1. Return ONLY the translated text — zero commentary, no extra quotes\n2. Preserve all formatting, line breaks, and punctuation exactly\n3. Maintain cultural nuance and natural fluency\n4. Keep brand names, proper nouns, and technical terms appropriate`;
  if (context) p += `\n\nCONTEXT: ${context}`;
...
/**
 * 1순위: DeepL Free Tier — API Endpoint: https://api-free.deepl.com/v2/translate
 * HTTP 456 (Quota Exceeded) 감지 → 즉시 err.code = 'DEEPL_QUOTA_EXCEEDED' 설정하여
 * 폴백 체인이 멈추지 않고 다음 엔진으로 자동 이동하도록 함
 */
async function translateWithDeepL(text, sourceLang, targetLang) {
  ...
  const deeplLangMap = {
    ...
    'pt': 'PT',  'pt-BR': 'PT-BR', 'it': 'IT', 'nl': 'NL',
    'pl': 'PL',  'ru': 'RU',  'ja': 'JA',  'zh-CN': 'ZH', 'ko': 'KO',
    'tr': 'TR',  'sv': 'SV',  'da': 'DA',  'fi': 'FI',  'nb': 'NB',
    'cs': 'CS',  'sk': 'SK',  'sl': 'SL',  'ro': 'RO',  'hu': 'HU',
    'bg': 'BG',  'el': 'EL',  'et': 'ET',  'lv': 'LV',  'lt': 'LT',
    'uk': 'UK',  'id': 'ID',
  };

  const deeplTarget = deeplLangMap[targetLang];
  if (!deeplTarget) throw new Error(`DeepL does not support target language: ${targetLang}`);

  const params = new URLSearchParams({ auth_key: apiKey, text, target_lang: deeplTarget });
  const deeplSource = deeplLangMap[sourceLang];
  if (deeplSource && sourceLang !== 'auto') params.append('source_lang', deeplSource);
  ...
  // ★ 456 = Free Tier 월 할당량 초과 → LLM 폴백 트리거
  if (response.status === 456) {
    const err = new Error('DeepL Free Tier 월 할당량 초과 (HTTP 456) — LLM 폴백 시작');
    err.code = 'DEEPL_QUOTA_EXCEEDED';
    logger.warn('[DeepL] 할당량 초과 → Claude 3.5 Sonnet으로 자동 우회');
    throw err;
  }
...
/**
 * 폴백 2순위: Claude 3.5 Sonnet (Anthropic 네이티브 API)
 */
async function translateWithClaude35Sonnet(text, sourceLang, targetLang, context = '') {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error('Anthropic API key not configured');

  const prompt = buildTranslationPrompt(
    text,
    SUPPORTED_LANGUAGES[sourceLang] || sourceLang,
  ...
  if (!response.ok) throw new Error(`Claude 3.5 Sonnet error: ${response.status}`);
  const data = await response.json();
  const result = data.content?.[0]?.text?.trim();
  if (!result) throw new Error('Claude 3.5 Sonnet: empty response');
  return result;
}

/**
 * 폴백 3순위: GPT-4o (OpenAI 네이티브 API)
 */
async function translateWithGPT4o(text, sourceLang, targetLang, context = '') {
  ...
  if (!result) throw new Error('GPT-4o: empty response');
  return result;
}

/**
 * 폴백 4순위: Gemini 1.5 Flash (Google 네이티브 API)
 */
async function translateWithGemini15Flash(text, sourceLang, targetLang, context = '') {
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`,
    {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { maxOutputTokens: Math.min(Math.ceil(text.length * 2.5), 4000) },
      }),
      signal: AbortSignal.timeout(30000),
    }
  );
  if (!response.ok) throw new Error(`Gemini 1.5 Flash error: ${response.status}`);
  const data = await response.json();
  const result = data.candidates?.[0]?.content?.parts?.[0]?.text?.trim();
  if (!result) throw new Error('Gemini 1.5 Flash: empty response');
  return result;
}
...
async function detectLanguageAdvanced(text) {
  ...
  // AI 기반 정밀 감지 (generateWithBestModel 활용)
  try {
    const result = await generateWithBestModel(
      `Detect the language of the following text and respond with ONLY the ISO 639-1 language code (e.g., "en", "ko", "ja"). If uncertain, respond with "unknown".\n\n${text.substring(0, 300)}`,
      { maxTokens: 10, preferFree: true }
    );
    const detectedLang = result.text.trim().toLowerCase();
    if (SUPPORTED_LANGUAGES[detectedLang]) return detectedLang;
    return 'unknown';
  } catch (error) {
...
export async function translate(text, targetLang, sourceLang = 'auto', options = {}) {
  ...
  // 언어 감지 및 모든 외부 API 호출 BEFORE — 캐시 히트 시 즉시 반환
  // 비용: $0, 지연: ~0ms
  // ══════════════════════════════════════════════════════════════
  const zeroCostHit = getCached(text, targetLang);
  if (zeroCostHit) {
    logger.info(`[ZeroCostCache] HIT — ${targetLang} | ${Date.now() - startTime}ms | provider was: ${zeroCostHit.provider}`);
    return zeroCostHit.translation;
  }
  if (sourceLang === 'auto') {
  ...
  // ═══════════════════════════════════════════════════════════
  // 다이내믹 AI 라우터 — 6단 폴백 체인 (Phase 8)
  //   1순위: DeepL Free Tier      (전문 번역 엔진 — 최고 품질)
  //   2순위: Claude 3.5 Sonnet    ← DeepL 할당량 초과(456) 시 즉시 우회
  //   3순위: GPT-4o               ← Claude 실패 시
  //   4순위: Gemini 1.5 Flash     ← GPT-4o 실패 시
  //   5순위: AI Auto-Select       ← aiModelManager 동적 발견
  //   6순위: Google Translate     ← 최후 수단
  // ═══════════════════════════════════════════════════════════
  const providers = [
    { name: 'DeepL (Free Tier)',  fn: () => translateWithDeepL(text, sourceLang, targetLang) },
    { name: 'Claude 3.5 Sonnet', fn: () => translateWithClaude35Sonnet(text, sourceLang, targetLang, context) },
    { name: 'GPT-4o',            fn: () => translateWithGPT4o(text, sourceLang, targetLang, context) },
    { name: 'Gemini 1.5 Flash',  fn: () => translateWithGemini15Flash(text, sourceLang, targetLang, context) },
    { name: 'AI (Auto-Select)',  fn: () => translateWithAI(text, sourceLang, targetLang, context) },
  ...
  // Zero-Cost 캐시에 저장 — 다음 동일 요청 시 API 비용 $0
  setCached(text, targetLang, translation, { sourceLang, provider: usedProvider });

  // 기존 Redis/인메모리 2차 캐시에도 저장 (호환성 유지)
...
export async function evaluateTranslationQuality(sourceText, translatedText, sourceLang, targetLang) {
  try {
    const prompt = `You are a translation quality evaluator. Evaluate this translation and respond ONLY with JSON.

Source (${sourceLang}): ${sourceText}
Translation (${targetLang}): ${translatedText}

JSON format:
{"score": <0-100>, "accuracy": <0-100>, "fluency": <0-100>, "feedback": "<brief>"`;
    const result = await generateWithBestModel(prompt, { maxTokens: 150, preferFree: true });
    const jsonMatch = result.text.match(/\{.*\}/s);
    if (jsonMatch) return JSON.parse(jsonMatch[0]);
    return { score: 50, feedback: 'Evaluation failed (no JSON)' };
  } catch (error) {
...
export async function getSystemHealth() {
  ...
  // AI 모델(generateWithBestModel) 상태 확인
  try {
    await translateWithAI('test', 'en', 'ko');
    health.providers.ai = { status: 'operational' };
  } catch (error) {
    health.providers.ai = { status: 'error', message: error.message };
