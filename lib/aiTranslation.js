...
export async function translate(text, targetLang, sourceLang = 'auto', options = {}) {
  ...
  // 멀티 프로바이더 폴백 체인
  // 1순위: DeepL (번역 전문 엔진 — 자연스러움 최고)
  // 2순위: AI 모델매니저 (Gemini/Claude/Groq 등 자동 선택)
  // 3순위: Google Translate (폴백)
  const providers = [
    { name: 'DeepL', fn: () => translateWithDeepL(text, sourceLang, targetLang) },
    { name: 'AI', fn: () => translateWithAI(text, sourceLang, targetLang, context) },
