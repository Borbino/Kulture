...
async function analyzeCEOFeedbackAdvanced() {
  ...
  feedbacks.forEach((fb) => {
    ...
    // 포맷 선호도
    if (text.includes('기사')) analysis.preferredFormats.article = (analysis.preferredFormats.article || 0) + 1;
    if (text.includes('르포') || text.includes('현장')) analysis.preferredFormats.reportage = (analysis.preferredFormats.reportage || 0) + 1;
    if (text.includes('썰') || text.includes('재미')) analysis.preferredFormats.story = (analysis.preferredFormats.story || 0) + 1;

    // 톤 선호도
    if (text.includes('객관') || text.includes('중립')) analysis.preferredTone.objective = (analysis.preferredTone.objective || 0) + 1;
    if (text.includes('재미') || text.includes('흥미')) analysis.preferredTone.entertaining = (analysis.preferredTone.entertaining || 0) + 1;
    if (text.includes('감성') || text.includes('감동')) analysis.preferredTone.emotional = (analysis.preferredTone.emotional || 0) + 1;
...
function verifyContentQuality(content, issue) {
  ...
  checks.passed = checks.hasTitle && checks.hasBody && checks.hasConclusion && checks.hasSEOKeywords && checks.mentionsKeyword && checks.wordCount >= 300 && checks.readability >= 50 && checks.seoScore >= 60;
...
function calculateSEOScore(content) {
  ...
  if (content.seoKeywords.length > 0) {
    ...
    checks.keywordInTitle = content.seoKeywords.some((kw) => content.title.toLowerCase().includes(kw.toLowerCase()));
  }
...
export async function generateAdvancedContent(issue, format = 'article') {
  ...
  try {
    ...
    console.log(`[Advanced Content] Generation completed in ${elapsed}ms (Quality Score: ${qualityCheck.score}/100)`);
 ...
 */
function generateTemplateContent(issue, format) {
  const formatConfig = CONTENT_FORMATS[format];
