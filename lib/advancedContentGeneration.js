...
async function callHuggingFace(prompt) {
  ...
  const endApiCall = performanceMonitor.startApiCall('huggingface-api')
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (response.status === 401) {
        const error = new Error('HF API authentication failed - check HUGGINGFACE_API_TOKEN')
        endApiCall(false, error)
        throw error
      }

      if (response.status === 429) {
        const error = new Error('HF API rate limit exceeded - please wait before retrying')
        endApiCall(false, error)
        throw error
      }
      ...
      if (!response.ok) {
        ...
        const error = new Error(`HF API error: ${response.status} - ${errorText.substring(0, 100)}`)
        endApiCall(false, error)
        throw error
      }
...
export async function generateAdvancedContent(issue, format = 'article') {
  ...
  try {
    ...
    console.log(
      `[Advanced Content] Generation completed in ${elapsed}ms (Quality Score: ${qualityCheck.score}/100, Review: ${reviewReport.status})`
    )
