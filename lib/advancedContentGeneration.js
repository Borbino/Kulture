...
export async function generateAdvancedContent(issue, format = 'article') {
  logger.info('[AdvancedContent]', `Starting generation for "${issue.keyword}" (${format})`)
  ...
  try {
    ...
    try {
      ...
      logger.warn('[AdvancedContent]', `AI 호출 실패, 템플릿으로 폴백: ${aiError.message}`)
    ...
    if (!qualityCheck.passed && qualityCheck.score < 70) {
      logger.warn('[AdvancedContent]', 'Quality check failed, regenerating...')
      // 간단한 템플릿 사용
    ...
    logger.info('[AdvancedContent]',
      `완료 in ${elapsed}ms (Quality: ${qualityCheck.score}/100, AI: ${aiProvider}/${aiModelName}, Review: ${reviewReport.status})`
    )
    ...
    logger.error('[AdvancedContent]', `Generation failed: ${error.message}`)
    return {
      success: false,
      error: error.message,
    }
