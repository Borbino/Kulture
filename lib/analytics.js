...
/**
 * GA4 스크롤 깊이 이벤트 (단발 발사용)
 *
 * @param {string} slug        - 기사 slug
 * @param {number} depthPct    - 도달한 스크롤 깊이 % (예: 50, 90)
 */
export const trackScrollDepth = (slug, depthPct) => {
  const lang = typeof window !== 'undefined' ? document.documentElement.lang : 'unknown'
  if (typeof window !== 'undefined' && window.gtag) {
  ...
  logger.info('analytics', 'scroll_depth', { slug, depthPct, lang })
}

/**
 * 체류 시간 타이머 초기화
 * 30초 / 60초 / 120초 마일스톤에 GA4 `time_on_page` 이벤트 자동 발사
 * 페이지 이탈 전 반드시 반환된 cleanup 함수를 호출해야 타이머가 정리됨
 *
 * @param {string} slug - 기사 slug
 * @returns {() => void} cleanup 함수 (useEffect return 등에서 호출)
 */
export const initTimeOnPageTracking = slug => {
  if (typeof window === 'undefined') return () => {}

  const MILESTONES = [30, 60, 120] // 초 단위
  const timers = MILESTONES.map(sec =>
    setTimeout(() => {
      trackTimeOnPage(slug, sec, null)
    }, sec * 1000)
  )
...
/**
 * 스크롤 깊이 리스너 초기화
 * 50% / 90% 도달 시 GA4 `scroll_depth` 이벤트 발사 (각 임계값당 최대 1회)
 * 페이지 이탈 전 반드시 반환된 cleanup 함수를 호출해야 리스너가 해제됨
 *
 * @param {string} slug - 기사 slug
 * @returns {() => void} cleanup 함수 (useEffect return 등에서 호출)
 */
export const initScrollDepthTracking = slug => {
  if (typeof window === 'undefined') return () => {}
