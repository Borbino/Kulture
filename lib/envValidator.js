...
/**
 * Supabase 필수 환경변수 — 누락 시 빌드 즉각 중단
 */
const SUPABASE_REQUIRED_ENV_VARS = {
  NEXT_PUBLIC_SUPABASE_URL: '필수: Supabase 프로젝트 URL (예: https://xxxx.supabase.co)',
  NEXT_PUBLIC_SUPABASE_ANON_KEY: '필수: Supabase 익명(anon) 공개 키',
}
...
export function checkEnvironment() {
  // ── Supabase 즉각 중단 검증 (환경 무관) ─────────────────────────────────
  const supabaseErrors = []
  Object.entries(SUPABASE_REQUIRED_ENV_VARS).forEach(([key, description]) => {
    const value = process.env[key]
    if (!value || value.trim() === '' || value.includes('your_') || value.includes('xxx')) {
      supabaseErrors.push(`  • ${key}: ${description}`)
    }
  })
  if (supabaseErrors.length > 0) {
    const msg = [
      '',
      '🔴 FATAL: Supabase 필수 환경변수가 누락되었습니다. 빌드를 중단합니다.',
      '',
      ...supabaseErrors,
      '',
      '해결 방법:',
      '  1. 프로젝트 루트의 .env.local 파일에 아래 변수를 추가하세요.',
      '  2. Vercel/CI 환경의 경우 대시보드 > Settings > Environment Variables에서 설정하세요.',
