...
export function checkEnvironment() {
  ...
  if (!hasProvider) {
    console.warn('⚠️  번역 프로바이더가 구성되지 않았습니다. OPENAI_API_KEY, GOOGLE_TRANSLATE_API_KEY 또는 DEEPL_API_KEY 중 하나를 설정하세요.')
  }

  // REDIS_URL 형식 간단 검증
  if (process.env.REDIS_URL && !/^redis:\/\//.test(process.env.REDIS_URL)) {
    console.warn(`⚠️  REDIS_URL 형식이 올바르지 않은 것 같습니다: ${process.env.REDIS_URL}`)
  }
